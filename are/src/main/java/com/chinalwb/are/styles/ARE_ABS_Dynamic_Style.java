...
public abstract class ARE_ABS_Dynamic_Style<E extends AreDynamicSpan> extends ARE_ABS_Style<E> {
    ...
    protected void applyNewStyle(Editable editable, int start, int end, int currentStyle) {
        ...
        /*//todo: this is not a factory method
        if (currentStyle == 0){
//            E[] spans = editable.getSpans(start, end, clazzE);
//            editable.removeSpan(spans[0]);
            E[] es = editable.getSpans(detectStart, detectEnd, clazzE);
            for (E e : es){
        ...
        if (existingSpans != null && existingSpans.length > 0) {
            ...
            if (startSpanFeature == currentStyle && endSpanFeature == currentStyle) {
                editable.setSpan(newSpan(), startSpanStart, endSpanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE/*Spanned.SPAN_INCLUSIVE_INCLUSIVE*/);
            } else if (startSpanFeature == currentStyle) {
                editable.setSpan(newSpan(startSpanFeature), startSpanStart, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE/*Spanned.SPAN_INCLUSIVE_EXCLUSIVE*/);
                editable.setSpan(newSpan(endSpanFeature), end, endSpanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE/*Spanned.SPAN_EXCLUSIVE_INCLUSIVE*/);
            } else if (endSpanFeature == currentStyle) {
                editable.setSpan(newSpan(startSpanFeature), startSpanStart, start, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE/*Spanned.SPAN_INCLUSIVE_EXCLUSIVE*/);
                editable.setSpan(newSpan(endSpanFeature), start, endSpanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE/*Spanned.SPAN_EXCLUSIVE_INCLUSIVE*/);
            } else {
                editable.setSpan(newSpan(startSpanFeature), startSpanStart, start, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE/*Spanned.SPAN_INCLUSIVE_EXCLUSIVE*/);
                if (endSpanEnd > end) {
                    editable.setSpan(newSpan(endSpanFeature), end, endSpanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE/*Spanned.SPAN_EXCLUSIVE_INCLUSIVE*/);
            ...
            editable.setSpan(newSpan(), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE/*Spanned.SPAN_INCLUSIVE_INCLUSIVE*/);
        }
    }
