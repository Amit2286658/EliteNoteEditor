...
//        this.mEditText.useSoftwareLayerOnAndroid8();
        ...
        final int w = Util.getScreenWidthAndHeight(mContext)[0];
        final String path = Util.GetPathFromUri4kitkat.getPath(mContext, uri);
        Glide.with(mContext)
                .asBitmap()
                .load(path)
                ...
                .into(new CustomTarget<Bitmap>() {
                    ...
                    public void onResourceReady(Bitmap resource, Transition<? super Bitmap> transition) {
                        resource = Util.scaleBitmapToFitWidth(resource, w);
                        Bitmap play = BitmapFactory.decodeResource(mContext.getResources(), R.drawable.play);
                        Bitmap video = Util.mergeBitmaps(resource, play);
        ...
        //final boolean isDownscaledEnabled = sp.getBoolean("downscaled_bitmaps_preference_key", true);

        /*video = Util.scaleBitmapToFitWidth(video, w);

        AreVideoSpan videoSpan = new AreVideoSpan(mContext, video, path, videoUrl);
        insertSpan(videoSpan);*/
    }

    //todo: adding an interface to listen to the permission and related events, not factory included

    private static videoAddingListener mVideoAddingListener;

    public interface videoAddingListener{
        void onAddingVideo(final Uri uri, final String videoUrl);
    }

    public static void setOnVideoAddingListener(videoAddingListener videoAddingListener){
        mVideoAddingListener = videoAddingListener;
    }

    private static void confirmOnVideoAddingListener(final Uri uri, final String videoUrl){
        mVideoAddingListener.onAddingVideo(uri, videoUrl);
    }
    ...
    public void insertSpan(AreVideoSpan imageSpan) {
